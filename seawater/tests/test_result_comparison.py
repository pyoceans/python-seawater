...
def compare_results(name, function, args, values):
    ...
    # NOTE: Test only the first output when multiple outputs are present.
...
class OctaveResultComparison(unittest.TestCase):
    def setUp(self):
        ...
        st61 = np.loadtxt(rootpath.joinpath("data", station_61), **kw)
        st64 = np.loadtxt(rootpath.joinpath("data", station_64), **kw)

        latst = 36.0 + 40.03 / 60.0, 37.0 + 39.93 / 60.0
        lonst = -(70.0 + 59.59 / 60.0), -71.0
        ...
        self.values = {
            ...
            "units": "km",
            "lon": np.array([-30.0] * 6),
            "lat": np.linspace(-22, -21.0, 6),
            "length": np.array([100.0] * 6),
            "latst": np.array(latst),
            "lonst": np.array(lonst),
            "Sal": Sal,
            "Temp": Temp,
            "Pres": Pres,
            "Gpan": Gpan,
