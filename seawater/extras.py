# -*- coding: utf-8 -*-
from seawater import dens, pden, cor  # TODO
...
def shear(P, U, V=0):
    ...
    m,n      = P.shape #TODO *check where depth increases to find dimension
    iup      = np.arange(0,m-1)
    ilo      = np.arange(1,m)
    p_ave    = ( P[iup,:] + P[ilo,:] )/2. # TODO
    vel      = np.sqrt( U**2 + V**2 )
...
def test():
    ...
    #def help(): TODO: add a help function like the blovk below and paste to every test
    #if display_help
    #eval(['help ' module])
    #eval(['help ' submodules])
    #end
    #TODO: add file keyword
    f = open('out.txt','w') #TODO: remove hardcoded filename
    #delete test.txt
    print >>f,  'OUTPUT FROM THIS TEST WILL ALSO BE SAVED IN FILE test.txt' #TODO put filename keyword here too
    ...
    #TODO: add keywords and test here!
